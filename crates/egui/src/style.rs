...
pub enum FontSelection {
    ...
    /// Use a [`TextStyle`] to look up the [`FontStyle`] in [`Style::text_styles`].
    Style(TextStyle),
}
...
impl FontSelection {
    pub fn resolve(self, style: &Style) -> FontStyle {
        match self {
            Self::Default => {
                if let Some(override_font_id) = &style.override_font {
...
pub struct Style {
    ...
    /// On most widgets you can also set an explicit text style,
    /// which will take precedence over this.
    // TODO(valadaptive): does this make sense anymore? FontStyle includes bold, italics, etc
    pub override_font: Option<FontStyle>,
...
impl Style {
    pub fn ui(&mut self, ui: &mut crate::Ui) {
        ...
        crate::Grid::new("_options").show(ui, |ui| {
            ...
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ui.radio_value(override_font, None, "None");
                    if ui.radio(override_font.is_some(), "override").clicked() {
                        *override_font = Some(FontStyle::default());
                    }
                });
                if let Some(override_font) = override_font {
                    crate::introspection::font_id_ui(ui, override_font);
                }
            });
            ui.end_row();
